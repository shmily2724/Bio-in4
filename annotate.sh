...
if supports_bcftools_atomize; then
  echo "[DEBUG] bcftools hỗ trợ --atomize ✅"
else
  echo "[DEBUG] bcftools KHÔNG hỗ trợ --atomize ❌"
fi
# ========================== CẤU HÌNH ==========================
...
supports_bcftools_atomize() {
  # Lấy đường dẫn thật sự được dùng
  local exe
  exe="$(command -v bcftools || true)"
  [[ -n "$exe" ]] || return 1

  # Đọc help vào biến, KHÔNG dùng pipeline để tránh pipefail
  local help
  help="$("$exe" norm -h 2>&1 || true)"

  # Match chuỗi thay vì grep pipeline (an toàn với set -e/pipefail)
  case "$help" in
    *"--atomize"*) return 0 ;;
    *)             return 1 ;;
  esac
...
atomize_after_norm() {
  ...
  if [[ "$ATOMIZE" != "true" ]]; then
    echo "[$(TS)] ⏭️  Bỏ qua atomize (ATOMIZE=false)"; cp -f "$in_gz" "$out_gz"; tabix -f -p vcf "$out_gz" || true; return
  fi
  if supports_bcftools_atomize; then
