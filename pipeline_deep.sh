...
# ===================================================================
# --- BƯỚC 4: GỌI BIẾN THỂ (DEEPVARIANT) & CHÚ GIẢI (SNPEFF + SNPSIFT) ---
# ===================================================================
echo ""
echo "---=== BƯỚC 4: Bắt đầu Gọi và Chú giải biến thể (DeepVariant) ===---"

echo "[4.1] Gọi biến thể với DeepVariant (Docker)..."
docker run --rm \
  -u "$(id -u)":"$(id -g)" \
...
echo "DeepVariant hoàn tất."

echo "[4.1b] Index VCF/GVCF..."
tabix -f -p vcf "${DV_VCF}" || true
tabix -f -p vcf "${DV_GVCF}" || true

echo "[4.2] Chú giải biến thể với SnpEff + SnpSift..."
...
# [4] 1000 Genomes (thêm AF) → ĐỔI TÊN AF → KG_AF (nếu có bcftools)
